    ...
    <a-table :columns='columns' :data-source="getWatchlist" rowKey='symbol'>
      <span slot="actions" slot-scope="stockInfo">
        ...
        <a-popconfirm
          ...
          ok-text="Yes"
          cancel-text="No"
          @confirm="removeFromWatchlist(stockInfo)"
          @cancel="cancel"
        >
  ...
  export default {
    ...
    methods: {
      buyStock(stockInfo) {
        ...
        this.priceToPass = stockInfo['price']
        this.exchangeNameToPass = stockInfo['exchange']
        this.action = 'buy'
        this.openQuantitySelector()
      },

      removeFromWatchlist(stockInfo) {
        removeSecurity(this.getAccessToken, stockInfo['symbol'], stockInfo['exchange'])
      },
